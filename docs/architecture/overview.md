...
| **Reverse Proxy** | Caddy config | TLS termination, HTTP to HTTPS redirect, gzip, upstream health checks, rate limiting | Inspect or mutate request bodies |
...
`api/` importing from `infrastructure/db/` or `domain/` importing from `api/`.
...
Upload (receipt, document):
    Client  ->  POST /api/v1/files (multipart/form-data)
            ...
            ->  Infrastructure: boto3.put_object
                  to s3://<bucket>/orgs/<organisation_id>/<type>/<uuid>.<ext>
...
Download (view receipt):
    Client  ->  GET /api/v1/files/{id}/url
            ...
            ->  Client fetches object directly from S3 endpoint (no proxy)
```
...
| **Database** | RLS policy `USING (organisation_id = current_setting('app.current_organisation_id')::uuid)` on every tenant table; app role has no `BYPASSRLS` |
| **Background jobs** | `organisation_id` is a mandatory argument; `TenantJob` base class restores context before execution |
| **Object storage** | All object paths prefixed `orgs/<organisation_id>/`; presigned URL generation validates the tenant match |

**Superadmin access:** cross-tenant operations use a separate PostgreSQL role (`BYPASSRLS`),
a distinct JWT audience (`aud: cairnbooks-admin`), and an IP allowlist. The application role
used by normal API requests can never bypass RLS.
...
| I-4 | **Multi-currency storage** — amounts always stored with ISO 4217 `currency_code`; base-currency conversion recorded at post time, never retroactively changed | Domain model enforces non-nullable `currency_code` |
...
| # | Invariant | Enforcement |
|---|---|---|
| I-7 | Every query, mutation, background job, and stored object is scoped to a single `organisation_id` | RLS + TenantSession + TenantJob (see section 7) |
...
| I-13 | All configuration is via environment variables (12-factor); no secrets committed to version control | `.env.example` documents required vars; `.gitignore` excludes `.env` |
...
[ADR-0001-stack.md section 3](./ADR-0001-stack.md#3-stack-rationale).
